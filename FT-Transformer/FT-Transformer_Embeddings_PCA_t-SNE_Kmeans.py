...
# Define the FT-Transformer model
device = 'cuda' if torch.cuda.is_available() else 'cpu'
model = FTTransformer(
    categories=(2,),  # Number of unique values in each categorical column
    num_continuous=len(numerical_columns),
    dim=128,  # Embedding dimension size
    dim_out = 32,                        # binary prediction, but could be anything
...
with torch.no_grad():
    for batch_categ, batch_cont in embedding_loader:
        ...
        embeddings = model(batch_categ, batch_cont)
